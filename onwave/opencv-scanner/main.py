...
def get_yolo_models():
    """Lazy-load the YOLO best_a4.pt model for A4 document detection.
    Returns list of (model, name, type) tuples — OnWave only uses best_a4.pt.
    """
    global _yolo_model
    if _yolo_model is None:
        try:
            ...
            if os.path.exists("best_a4.pt"):
                logger.info("Loading YOLO A4 model: best_a4.pt")
                models.append((YOLO("best_a4.pt"), "best_a4.pt", "classic"))
            
            if not models:
                logger.warning("best_a4.pt not found. OpenCV fallback will be used.")
...
def yolo_crop_document(img: np.ndarray, conf_threshold: float = 0.3, margin_pct: float = 0.05) -> List[np.ndarray]:
    """
    Use YOLO best_a4.pt to detect and crop A4 documents (invoices, bank statements)
    from a photo or scan. Returns: List of cropped images. Empty list if none detected.
    
    OnWave-specific: simplified for A4 sheets only (no OBB, no ID cards, no multi-model).
    """
    models = get_yolo_models()
    if not models:
        logger.warning("YOLO: best_a4.pt not available, skipping")
        return []
    
    h, w = img.shape[:2]
    all_boxes = []
    
    for model, model_source, model_type in models:
        try:
            ...
            if boxes is None or len(boxes) == 0:
                logger.info(f"YOLO [{model_source}]: 0 detections")
                continue
            
            logger.info(f"YOLO [{model_source}]: {len(boxes)} detection(s)")
            ...
            for box in boxes:
                ...
                all_boxes.append((x1, y1, x2, y2, conf, cls_name, model_source))
        except Exception as e:
            logger.warning(f"YOLO inference failed for {model_source}: {e}")
            continue

    if not all_boxes:
        logger.info("YOLO: no valid A4 detections found")
        return []

    # Sort by confidence, keep best non-overlapping
    ...
    # Sort top-to-bottom for consistent page ordering
    final_boxes.sort(key=lambda b: b[1])

    # Smart merge: if exactly 2 boxes are vertically stacked with small gap, merge them
    # (common when a single A4 page is folded or split by the model)
    if len(final_boxes) == 2:
        b1, b2 = final_boxes
        gap = float(b2[1] - b1[3])
        min_h = min(float(b1[3] - b1[1]), float(b2[3] - b2[1]))
        adaptive_threshold = max(float(SMART_MERGE_GAP_THRESHOLD), 0.25 * min_h)
        
        # Check X-axis alignment
        x_overlap_start = max(float(b1[0]), float(b2[0]))
        x_overlap_end = min(float(b1[2]), float(b2[2]))
        x_overlap_len = max(0, x_overlap_end - x_overlap_start)
        min_width = min(float(b1[2] - b1[0]), float(b2[2] - b2[0]))
        x_overlap_ratio = x_overlap_len / max(1.0, min_width)

        if 0 <= gap < adaptive_threshold and x_overlap_ratio >= 0.70:
            merged = (
                min(b1[0], b2[0]), min(b1[1], b2[1]),
                max(b1[2], b2[2]), max(b1[3], b2[3]),
                max(b1[4], b2[4]), "merged", "smart-merge"
            )
            logger.info(f"YOLO smart-merge: 2 boxes → 1 (gap={gap:.1f}px, xov={x_overlap_ratio:.2f})")
            final_boxes = [merged]

    # Crop each detected A4 region
    crops = []
    for x1, y1, x2, y2, conf, cls_name, source in final_boxes:
        box_w = x2 - x1
        box_h = y2 - y1
        ...
        if cropped.size > 0:
            ...
            # A4 doc should be at least 200px on each side
            if cw_crop < 200 or ch_crop < 200:
                logger.warning(f"YOLO: crop too small ({cw_crop}x{ch_crop}), rejecting")
                continue
            crop_gray = cv2.cvtColor(cropped, cv2.COLOR_BGR2GRAY)
            if crop_gray.std() >= 10.0:
                logger.info(f"YOLO A4 crop: {cls_name} (conf={conf:.2f}) → {cw_crop}x{ch_crop}")
                crops.append(cropped)
            else:
                logger.warning(f"YOLO: crop looks blank (std={crop_gray.std():.1f}), rejecting")
...
# ==========================================
# ONWAVE: RECONCILIATION & ACCOUNTING ENGINE
# ==========================================
# This module is proprietary and loaded separately.
# Contact: fennoune.me
# ==========================================
try:
    from reconcile_engine import router as _reconcile_router
    app.include_router(_reconcile_router)
except ImportError:
    pass

@app.get('/health')
def health():
    return {'status': 'ok', 'engine': 'OnWave', 'accepts': 'single file (pdf/jpg/png) + reconciliation', 'output': 'base64 images OR accounting entries'}
